...
import { test as tauriTest, expect as tauriExpect } from './fixtures/tauri-mock';
import { addTransaction, onboard } from './helpers/ui';
import type { Page } from '@playwright/test';

/** Count transactions in the database via the test hook. */
async function countTransactions(page: Page): Promise<number> {
	return page.evaluate(async () => {
		const hooks = (window as unknown as { __notchyTestHooks?: { getDb: () => Promise<{ query: (sql: string) => Promise<Array<{ cnt: number }>> }> } }).__notchyTestHooks;
...
tauriTest.describe('CSV import — database verification', () => {
	tauriTest('re-importing the same file writes zero new rows', async ({ tauriMockPage: page }) => {
		await onboard(page);
		await page.getByRole('link', { name: 'Transactions', exact: true }).click();
		await page.getByRole('button', { name: 'Import Transactions' }).click();
		const modal = page.getByRole('dialog');
		...
		await modal.getByRole('button', { name: 'Continue to preview' }).click();
		await modal.getByRole('button', { name: /Import \d+/ }).click();
		await tauriExpect(page.getByRole('dialog')).toHaveCount(0);

		// Count transactions in the database after the first import.
		const countBefore = await countTransactions(page);
		...
		await tauriExpect(modal2.getByText(/1 duplicate/)).toBeVisible();
		await tauriExpect(modal2.getByText(/0 new/)).toBeVisible();

		// Verify at the database level that zero rows were written.
		const countAfter = await countTransactions(page);
		tauriExpect(countAfter).toBe(countBefore);
	});
});
